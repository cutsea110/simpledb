...
impl fmt::Display for SetDateRecord {
...
impl LogRecord for SetDateRecord {
    ...
    fn undo(&mut self, tx: Arc<Mutex<Transaction>>) -> Result<()> {
        ...
        tx.lock()
            .unwrap()
            .set_date(&self.blk, self.offset, self.val, false)?; // don't log the undo!
...
impl SetDateRecord {
    ...
    pub fn write_to_log(
        ...
        p.set_i32(0, TxType::SETDATE as i32)?;
