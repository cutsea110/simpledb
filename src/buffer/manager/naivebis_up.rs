...
#[derive(Debug, Clone)]
pub struct NaiveBisUPBufferMgr {
...
mod tests {
    ...
    #[test]
    fn unit_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/naivebisup").exists() {
            fs::remove_dir_all("_test/buffermgrtest/naivebisup")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/naivebisup",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = NaiveBisUPBufferMgr::new(fm, lm, 3);
    ...
    fn replace_strategy_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/naivebisupstrategy").exists() {
            fs::remove_dir_all("_test/buffermgrtest/naivebisupstrategy")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/naivebisupstrategy",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = NaiveBisUPBufferMgr::new(fm, lm, 4);
    ...
    fn replace_strategy_modified_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/naivebisupstrategy_all_modified").exists() {
            fs::remove_dir_all("_test/buffermgrtest/naivebisupstrategy_all_modified")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/naivebisupstrategy_all_modified",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));
