...
impl FifoUPBufferMgr {
    ...
    // The FIFO with Unmodified Preferd Strategy
    fn choose_unpinned_buffer(&mut self) -> Option<(usize, Arc<Mutex<Buffer>>)> {
        ...
        for i in 0..self.assigned_buffers.len() {
            let buff = self.assigned_buffers[i].lock().unwrap();

            if !buff.is_pinned() {
                if !buff.is_modified() {
                    // find unmodified page
                    return Some((i, Arc::clone(&self.assigned_buffers[i])));
                } else if first_modified.is_none() {
                    // hold first modified page
                    first_modified = Some((i, Arc::clone(&self.assigned_buffers[i])));
                }
            }
        }
...
impl BufferMgr for FifoUPBufferMgr {
    // synchronized
...
mod tests {
    ...
    #[test]
    fn unit_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/fifoup").exists() {
            fs::remove_dir_all("_test/buffermgrtest/fifoup")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new("_test/buffermgrtest/fifoup", 400)?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = FifoUPBufferMgr::new(fm, lm, 3);
    ...
    fn replace_strategy_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/fifoupstrategy").exists() {
            fs::remove_dir_all("_test/buffermgrtest/fifoupstrategy")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/fifoupstrategy",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = FifoUPBufferMgr::new(fm, lm, 4);
    ...
    fn replace_strategy_modified_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/fifoupstrategy_all_modified").exists() {
            fs::remove_dir_all("_test/buffermgrtest/fifoupstrategy_all_modified")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/fifoupstrategy_all_modified",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));
        ...
        buff[1] = None;
        buff[4] = bm.pin(&BlockId::new("testfile", 50))?.into();
        bm.unpin(Arc::clone(&buff[3].clone().unwrap()))?; // unpin 40 without modified
        buff[3] = None;
