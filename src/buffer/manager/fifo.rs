...
//
//  (5) unpin
//      enqueue unassigned_buffers, if the buffer hasn't pinned.
//
#[derive(Debug, Clone)]
pub struct FifoBufferMgr {
...
impl FifoBufferMgr {
    pub fn new(fm: Arc<Mutex<FileMgr>>, lm: Arc<Mutex<LogMgr>>, numbuffs: usize) -> Self {
        ...
        Self {
            ...
            unassigned_buffers: VecDeque::from_iter(0..numbuffs),
        }
    }
    ...
    fn find_existing_buffer(&mut self, blk: &BlockId) -> Option<usize> {
        if let Some(i) = self.assigned_buffers.get(blk) {
            if !self.bufferpool[*i].lock().unwrap().is_pinned() {
                // TODO: I'd like to retain_first.
                self.unassigned_buffers.retain(|j| i != j);
            }
            return Some(*i);
        }
            ...
            *(self.num_available.lock().unwrap()) += 1;
            ...
            if let Some(blk) = b.block() {
                if let Some(idx) = self.assigned_buffers.get(blk) {
                    self.unassigned_buffers.push_back(*idx);
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn unit_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/fifo").exists() {
            fs::remove_dir_all("_test/buffermgrtest/fifo")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new("_test/buffermgrtest/fifo", 400)?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = FifoBufferMgr::new(fm, lm, 3);
    ...
    fn replace_strategy_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/fifostrategy").exists() {
            fs::remove_dir_all("_test/buffermgrtest/fifostrategy")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/fifostrategy",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = FifoBufferMgr::new(fm, lm, 4);
