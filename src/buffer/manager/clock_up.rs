...
mod tests {
    ...
    #[test]
    fn unit_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/clockup").exists() {
            fs::remove_dir_all("_test/buffermgrtest/clockup")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/clockup",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = ClockUPBufferMgr::new(fm, lm, 3);
    ...
    fn replace_strategy_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/clockupstrategy").exists() {
            fs::remove_dir_all("_test/buffermgrtest/clockupstrategy")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/clockupstrategy",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));

        let mut bm = ClockUPBufferMgr::new(fm, lm, 4);
    ...
    fn replace_strategy_modified_test() -> Result<()> {
        if Path::new("_test/buffermgrtest/clockupstrategy_all_modified").exists() {
            fs::remove_dir_all("_test/buffermgrtest/clockupstrategy_all_modified")?;
        }

        let fm = Arc::new(Mutex::new(FileMgr::new(
            "_test/buffermgrtest/clockupstrategy_all_modified",
            400,
        )?));
        let lm = Arc::new(Mutex::new(LogMgr::new(Arc::clone(&fm), "simpledb.log")?));
